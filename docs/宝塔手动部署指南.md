...
1. **宝塔面板 → 网站 → 添加站点**
   - 域名：`api.ai80.vip`（替换为你的域名）
   - PHP版本：纯静态
   - 其他默认

2. **进入站点目录**：
   ```bash
   cd /www/wwwroot/api.ai80.vip
   ```
...
# 上传到服务器
scp dist.tar.gz root@你的服务器IP:/www/wwwroot/api.ai80.vip/web/
```

**服务器执行**：
```bash
cd /www/wwwroot/api.ai80.vip/web
...
2. **填写项目信息**：
   ...
   | 字段 | 填写方式 |
   |------|----------|
   | 项目执行文件 | 点击右侧文件夹图标 → 进入 `/www/wwwroot/api.ai80.vip/` → 选择 `yi-api` 文件 |
   | 项目名称 | `yi-api` |
   | 项目端口 | `3000` |
   | 执行命令 | 留空 |
   | 环境变量 | 选择 **从文件加载** |
   | 环境变量文件路径 | `/www/wwwroot/api.ai80.vip/.env` |
   | 运行用户 | `www` |
   | 开机启动 | ✅ 勾选 |
   | 绑定域名 | `api.ai80.vip` |
...
```bash
# 创建部署脚本
cat > /www/wwwroot/api.ai80.vip/deploy.sh << 'EOF'
#!/bin/bash
cd /www/wwwroot/api.ai80.vip
git pull
cd web && npm run build && cd ..
...
chmod +x /www/wwwroot/api.ai80.vip/deploy.sh
...
```bash
# 确保 www 用户有权限
chown -R www:www /www/wwwroot/api.ai80.vip
chmod +x /www/wwwroot/api.ai80.vip/yi-api
```
